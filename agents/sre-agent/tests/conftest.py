...
agent construction in ``src.agent.agent`` (``RCA_AGENT``/``REMED_AGENT``/
``CHAT_AGENT`` each call ``get_model()`` in ``Agent.__init__``) succeed without
a real LLM backend or network access.
