...
func TestNewSecretReferenceCmd_Structure(t *testing.T) {
	cmd := NewSecretReferenceCmd()

	assert.Equal(t, "secretreference", cmd.Use)
	assert.Contains(t, cmd.Aliases, "sr")
	assert.Contains(t, cmd.Aliases, "secretreferences")
	assert.Contains(t, cmd.Aliases, "secretref")
...
func TestSecretReferenceCmd_GetAndDeleteRequireArg(t *testing.T) {
	cmd := NewSecretReferenceCmd()
	...
	for _, name := range []string{"get", "delete"} {
		t.Run(name, func(t *testing.T) {
			...
			assert.NoError(t, subCmd.Args(subCmd, []string{"my-secret"}))
		})
	}
}

func TestSecretReferenceCmd_NamespaceFlag(t *testing.T) {
	cmd := NewSecretReferenceCmd()
