...
func TestNewComponentTypeCmd_Structure(t *testing.T) {
	cmd := NewComponentTypeCmd()
...
func TestComponentTypeCmd_GetAndDeleteRequireArg(t *testing.T) {
	cmd := NewComponentTypeCmd()
	...
	for _, name := range []string{"get", "delete"} {
		t.Run(name, func(t *testing.T) {
			...
			assert.NoError(t, subCmd.Args(subCmd, []string{"my-componenttype"}))
		})
	}
}

func TestComponentTypeCmd_NamespaceFlag(t *testing.T) {
	cmd := NewComponentTypeCmd()
