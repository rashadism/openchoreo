...
func TestNewProjectCmd_Structure(t *testing.T) {
	cmd := NewProjectCmd()
...
func TestProjectCmd_GetAndDeleteRequireArg(t *testing.T) {
	cmd := NewProjectCmd()
	...
	for _, name := range []string{"get", "delete"} {
		t.Run(name, func(t *testing.T) {
			...
			assert.NoError(t, subCmd.Args(subCmd, []string{"my-project"}))
		})
	}
}

func TestProjectCmd_NamespaceFlag(t *testing.T) {
	cmd := NewProjectCmd()
