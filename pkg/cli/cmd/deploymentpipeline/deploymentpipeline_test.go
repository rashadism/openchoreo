...
func TestNewDeploymentPipelineCmd_Structure(t *testing.T) {
	cmd := NewDeploymentPipelineCmd()
...
func TestDeploymentPipelineCmd_GetAndDeleteRequireArg(t *testing.T) {
	cmd := NewDeploymentPipelineCmd()
	...
	for _, name := range []string{"get", "delete"} {
		t.Run(name, func(t *testing.T) {
			...
			assert.NoError(t, subCmd.Args(subCmd, []string{"my-pipeline"}))
		})
	}
}

func TestDeploymentPipelineCmd_NamespaceFlag(t *testing.T) {
	cmd := NewDeploymentPipelineCmd()
