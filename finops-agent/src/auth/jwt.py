...
class JWTValidator:
        ...
        with self._lock:
            if self._jwks_client is None or (now - self._last_refresh) > self.refresh_interval:
                ...
                self._jwks_client = PyJWKClient(
                    ...
                    headers={"User-Agent": "finops-agent/1.0"},
    ...
    def validate(self, token: str) -> dict[str, Any]:
        try:
            ...
            logger.warning("Failed to fetch signing key from JWKS", extra={"error_type": type(e).__name__})
            raise JWTValidationError("Failed to fetch signing key") from e
            ...
            logger.warning("Invalid token", extra={"error_type": type(e).__name__})
            raise JWTValidationError("Invalid token") from e
