# Configuring the Workflow Plane

The Workflow Plane executes CI workflows using Argo Workflows. Built container images are pushed to a container registry.

## Container Registry

The Workflow Plane requires a container registry to store built images. You have two options:
...
Then configure the workflow plane:

```yaml
global:
  defaultResources:
    registry:
      host: "registry.openchoreo-workflow-plane.svc.cluster.local:5000"
      tlsVerify: false
```
...
```bash
helm install openchoreo-workflow-plane oci://ghcr.io/openchoreo/helm-charts/openchoreo-workflow-plane \
  --namespace openchoreo-workflow-plane \
  --create-namespace \
