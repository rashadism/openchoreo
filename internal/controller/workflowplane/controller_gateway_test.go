...
var _ = Describe("WorkflowPlane Controller — gateway paths", func() {

	Describe("Create reconcile path", func() {
		const wpName = "wp-gw-create"
	...
	Describe("Spec-change re-notification (shouldIgnoreReconcile=true)", func() {
		const wpName = "wp-gw-spec-change"
		nn := types.NamespacedName{Name: wpName, Namespace: "default"}
		...
		It("re-notifies gateway when generation has advanced beyond ObservedGeneration", func() {
			...
			Expect(k8sClient.Get(ctx, nn, wp)).To(Succeed())
			wp.Spec.ClusterAgent.ClientCA.Value = "updated-ca-cert"
			Expect(k8sClient.Update(ctx, wp)).To(Succeed())
			Expect(k8sClient.Get(ctx, nn, wp)).To(Succeed())
			Expect(wp.Generation).To(BeNumerically(">", wp.Status.ObservedGeneration))
