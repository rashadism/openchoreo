...
func (s *ComponentService) GetComponentReleaseSchema(ctx context.Context, namespaceName, projectName, componentName, releaseName string) (*extv1.JSONSchemaProps, error) {
	...
	if release.Spec.ComponentProfile != nil {
		for _, componentTrait := range release.Spec.ComponentProfile.Traits {
			...
			traitJSONSchema, err := s.buildTraitEnvironmentConfigsSchema(traitSpec, componentTrait.Name)
			if err != nil {
				return nil, err
			}
...
func (s *ComponentService) PatchReleaseBinding(ctx context.Context, namespaceName, projectName, componentName, bindingName string, req *models.PatchReleaseBindingRequest) (*models.ReleaseBindingResponse, error) {
	...
	if req.ComponentTypeEnvironmentConfigs != nil {
		envConfigsJSON, err := json.Marshal(req.ComponentTypeEnvironmentConfigs)
		if err != nil {
			s.logger.Error("Failed to marshal component type environment configs", "error", err)
			return nil, fmt.Errorf("failed to marshal component type environment configs: %w", err)
		}
...
func (s *ComponentService) toReleaseBindingResponse(binding *openchoreov1alpha1.ReleaseBinding, namespaceName, projectName, componentName string) *models.ReleaseBindingResponse {
	...
	if binding.Spec.ComponentTypeEnvironmentConfigs != nil {
		...
		if err := json.Unmarshal(binding.Spec.ComponentTypeEnvironmentConfigs.Raw, &overrides); err == nil {
			response.ComponentTypeEnvironmentConfigs = overrides
		}
	}
...
func (s *ComponentService) validateWorkflowParameters(ctx context.Context, namespaceName string, workflowKind openchoreov1alpha1.WorkflowRefKind, workflowName string, providedParameters *runtime.RawExtension) error {
	...
	if workflowSpec.Parameters.GetRaw() == nil {
		return nil
	}
