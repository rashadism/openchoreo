...
// --- NewTraitCmd structure ---

func TestNewTraitCmd_Use(t *testing.T) {
	cmd := NewTraitCmd(errFactory("unused"))
	assert.Equal(t, "trait", cmd.Use)
	assert.Contains(t, cmd.Aliases, "traits")
}

func TestNewTraitCmd_Subcommands(t *testing.T) {
	cmd := NewTraitCmd(errFactory("unused"))
...
func TestListCmd_Success(t *testing.T) {
	...
	mc.EXPECT().ListTraits(mock.Anything, mock.Anything, mock.Anything).Return(&gen.TraitList{
...
func TestGetCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().GetTrait(mock.Anything, mock.Anything, "ingress").Return(
		&gen.Trait{Metadata: gen.ObjectMeta{Name: "ingress"}}, nil,
	)
...
func TestDeleteCmd_MissingArg(t *testing.T) {
	...
	err := cmd.Args(cmd, []string{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "TRAIT_NAME")
}
...
func TestDeleteCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().DeleteTrait(mock.Anything, mock.Anything, "ingress").Return(nil)
