...
// --- NewClusterDataPlaneCmd structure ---

func TestNewClusterDataPlaneCmd_Use(t *testing.T) {
	cmd := NewClusterDataPlaneCmd(errFactory("unused"))
	assert.Equal(t, "clusterdataplane", cmd.Use)
	assert.Contains(t, cmd.Aliases, "cdp")
	assert.Contains(t, cmd.Aliases, "clusterdataplanes")
}

func TestNewClusterDataPlaneCmd_Subcommands(t *testing.T) {
	cmd := NewClusterDataPlaneCmd(errFactory("unused"))
...
func TestListCmd_Success(t *testing.T) {
	...
	mc.EXPECT().ListClusterDataPlanes(mock.Anything, mock.Anything).Return(&gen.ClusterDataPlaneList{
		Items:      []gen.ClusterDataPlane{{Metadata: gen.ObjectMeta{Name: "default"}}},
...
func TestGetCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().GetClusterDataPlane(mock.Anything, "default").Return(
		&gen.ClusterDataPlane{Metadata: gen.ObjectMeta{Name: "default"}}, nil,
	)
...
func TestDeleteCmd_MissingArg(t *testing.T) {
	...
	err := cmd.Args(cmd, []string{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "CLUSTER_DATA_PLANE_NAME")
}
...
func TestDeleteCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().DeleteClusterDataPlane(mock.Anything, "default").Return(nil)
