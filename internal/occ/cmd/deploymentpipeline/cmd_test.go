...
// --- NewDeploymentPipelineCmd structure ---

func TestNewDeploymentPipelineCmd_Use(t *testing.T) {
	cmd := NewDeploymentPipelineCmd(errFactory("unused"))
	assert.Equal(t, "deploymentpipeline", cmd.Use)
	assert.Contains(t, cmd.Aliases, "deppipe")
	assert.Contains(t, cmd.Aliases, "deploymentpipelines")
}

func TestNewDeploymentPipelineCmd_Subcommands(t *testing.T) {
	cmd := NewDeploymentPipelineCmd(errFactory("unused"))
...
func TestListCmd_Success(t *testing.T) {
	...
	mc.EXPECT().ListDeploymentPipelines(mock.Anything, mock.Anything, mock.Anything).Return(&gen.DeploymentPipelineList{
		Items:      []gen.DeploymentPipeline{{Metadata: gen.ObjectMeta{Name: "my-pipeline"}}},
...
func TestGetCmd_FactoryError(t *testing.T) {
	...
	err := cmd.RunE(cmd, []string{"my-pipeline"})
	assert.EqualError(t, err, "factory failed")
}

func TestGetCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().GetDeploymentPipeline(mock.Anything, mock.Anything, "my-pipeline").Return(
		&gen.DeploymentPipeline{Metadata: gen.ObjectMeta{Name: "my-pipeline"}}, nil,
	)

	cmd := newGetCmd(mockFactory(mc))
	require.NoError(t, cmd.Flags().Set("namespace", "acme-corp"))
	out := testutil.CaptureStdout(t, func() {
		require.NoError(t, cmd.RunE(cmd, []string{"my-pipeline"}))
	})
	assert.Contains(t, out, "my-pipeline")
}
...
func TestDeleteCmd_MissingArg(t *testing.T) {
	...
	err := cmd.Args(cmd, []string{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "DEPLOYMENT_PIPELINE_NAME")
}
...
func TestDeleteCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().DeleteDeploymentPipeline(mock.Anything, mock.Anything, "my-pipeline").Return(nil)
