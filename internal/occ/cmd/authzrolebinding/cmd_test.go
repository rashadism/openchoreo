...
// --- NewAuthzRoleBindingCmd structure ---

func TestNewAuthzRoleBindingCmd_Use(t *testing.T) {
	cmd := NewAuthzRoleBindingCmd(errFactory("unused"))
	assert.Equal(t, "authzrolebinding", cmd.Use)
	assert.Contains(t, cmd.Aliases, "arb")
	assert.Contains(t, cmd.Aliases, "authzrolebindings")
}

func TestNewAuthzRoleBindingCmd_Subcommands(t *testing.T) {
	cmd := NewAuthzRoleBindingCmd(errFactory("unused"))
...
func TestListCmd_Success(t *testing.T) {
	...
	mc.EXPECT().ListNamespaceRoleBindings(mock.Anything, mock.Anything, mock.Anything).Return(&gen.AuthzRoleBindingList{
		Items:      []gen.AuthzRoleBinding{{Metadata: gen.ObjectMeta{Name: "my-binding"}}},
...
func TestGetCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().GetNamespaceRoleBinding(mock.Anything, mock.Anything, "my-binding").Return(
		&gen.AuthzRoleBinding{Metadata: gen.ObjectMeta{Name: "my-binding"}}, nil,
	)
...
func TestDeleteCmd_MissingArg(t *testing.T) {
	...
	err := cmd.Args(cmd, []string{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "AUTHZ_ROLE_BINDING_NAME")
}
...
func TestDeleteCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().DeleteNamespaceRoleBinding(mock.Anything, mock.Anything, "my-binding").Return(nil)
