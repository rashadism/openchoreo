...
// --- NewClusterWorkflowPlaneCmd structure ---

func TestNewClusterWorkflowPlaneCmd_Use(t *testing.T) {
	cmd := NewClusterWorkflowPlaneCmd(errFactory("unused"))
	assert.Equal(t, "clusterworkflowplane", cmd.Use)
	assert.Contains(t, cmd.Aliases, "cwp")
	assert.Contains(t, cmd.Aliases, "clusterworkflowplanes")
}

func TestNewClusterWorkflowPlaneCmd_Subcommands(t *testing.T) {
	cmd := NewClusterWorkflowPlaneCmd(errFactory("unused"))
...
func TestListCmd_Success(t *testing.T) {
	...
	mc.EXPECT().ListClusterWorkflowPlanes(mock.Anything, mock.Anything).Return(&gen.ClusterWorkflowPlaneList{
		Items:      []gen.ClusterWorkflowPlane{{Metadata: gen.ObjectMeta{Name: "default"}}},
...
func TestGetCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().GetClusterWorkflowPlane(mock.Anything, "default").Return(
		&gen.ClusterWorkflowPlane{Metadata: gen.ObjectMeta{Name: "default"}}, nil,
	)
...
func TestDeleteCmd_MissingArg(t *testing.T) {
	...
	err := cmd.Args(cmd, []string{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "CLUSTER_WORKFLOW_PLANE_NAME")
}
...
func TestDeleteCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().DeleteClusterWorkflowPlane(mock.Anything, "default").Return(nil)
