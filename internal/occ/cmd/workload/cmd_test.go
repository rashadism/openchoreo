...
// --- NewWorkloadCmd structure ---

func TestNewWorkloadCmd_Use(t *testing.T) {
	cmd := NewWorkloadCmd(errFactory("unused"))
	assert.Equal(t, "workload", cmd.Use)
	assert.Contains(t, cmd.Aliases, "wl")
	assert.Contains(t, cmd.Aliases, "workloads")
}

func TestNewWorkloadCmd_Subcommands(t *testing.T) {
	cmd := NewWorkloadCmd(errFactory("unused"))
	names := make([]string, 0, len(cmd.Commands()))
	for _, sub := range cmd.Commands() {
		names = append(names, sub.Name())
	}
	assert.ElementsMatch(t, []string{"create", "list", "get", "delete"}, names)
}
...
func TestListCmd_Success(t *testing.T) {
	...
	mc.EXPECT().ListWorkloads(mock.Anything, mock.Anything, mock.Anything).Return(&gen.WorkloadList{
		Items:      []gen.Workload{{Metadata: gen.ObjectMeta{Name: "my-workload"}}},
	...
	assert.Contains(t, out, "my-workload")
}
...
func TestGetCmd_FactoryError(t *testing.T) {
	...
	err := cmd.RunE(cmd, []string{"my-workload"})
	assert.EqualError(t, err, "factory failed")
}

func TestGetCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().GetWorkload(mock.Anything, mock.Anything, "my-workload").Return(
		&gen.Workload{Metadata: gen.ObjectMeta{Name: "my-workload"}}, nil,
	)

	cmd := newGetCmd(mockFactory(mc))
	require.NoError(t, cmd.Flags().Set("namespace", "acme-corp"))
	out := testutil.CaptureStdout(t, func() {
		require.NoError(t, cmd.RunE(cmd, []string{"my-workload"}))
	})
	assert.Contains(t, out, "my-workload")
}
...
func TestDeleteCmd_MissingArg(t *testing.T) {
	...
	err := cmd.Args(cmd, []string{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "WORKLOAD_NAME")
}
...
func TestDeleteCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().DeleteWorkload(mock.Anything, mock.Anything, "my-workload").Return(nil)
