...
// --- NewWorkflowPlaneCmd structure ---

func TestNewWorkflowPlaneCmd_Use(t *testing.T) {
	cmd := NewWorkflowPlaneCmd(errFactory("unused"))
	assert.Equal(t, "workflowplane", cmd.Use)
	assert.Contains(t, cmd.Aliases, "wp")
	assert.Contains(t, cmd.Aliases, "workflowplanes")
}

func TestNewWorkflowPlaneCmd_Subcommands(t *testing.T) {
	cmd := NewWorkflowPlaneCmd(errFactory("unused"))
...
func TestListCmd_Success(t *testing.T) {
	...
	mc.EXPECT().ListWorkflowPlanes(mock.Anything, mock.Anything, mock.Anything).Return(&gen.WorkflowPlaneList{
		Items:      []gen.WorkflowPlane{{Metadata: gen.ObjectMeta{Name: "primary"}}},
...
func TestGetCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().GetWorkflowPlane(mock.Anything, mock.Anything, "primary").Return(
		&gen.WorkflowPlane{Metadata: gen.ObjectMeta{Name: "primary"}}, nil,
	)
...
func TestDeleteCmd_MissingArg(t *testing.T) {
	...
	err := cmd.Args(cmd, []string{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "WORKFLOWPLANE_NAME")
}
...
func TestDeleteCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().DeleteWorkflowPlane(mock.Anything, mock.Anything, "primary").Return(nil)
