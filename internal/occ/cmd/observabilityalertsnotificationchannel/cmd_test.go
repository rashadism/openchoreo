...
// --- NewObservabilityAlertsNotificationChannelCmd structure ---

func TestNewObservabilityAlertsNotificationChannelCmd_Use(t *testing.T) {
	cmd := NewObservabilityAlertsNotificationChannelCmd(errFactory("unused"))
	assert.Equal(t, "observabilityalertsnotificationchannel", cmd.Use)
	assert.Contains(t, cmd.Aliases, "oanc")
}

func TestNewObservabilityAlertsNotificationChannelCmd_Subcommands(t *testing.T) {
	cmd := NewObservabilityAlertsNotificationChannelCmd(errFactory("unused"))
...
func TestListCmd_Success(t *testing.T) {
	...
	mc.EXPECT().ListObservabilityAlertsNotificationChannels(mock.Anything, mock.Anything, mock.Anything).Return(&gen.ObservabilityAlertsNotificationChannelList{
		Items:      []gen.ObservabilityAlertsNotificationChannel{{Metadata: gen.ObjectMeta{Name: "slack-channel"}}},
	...
	assert.Contains(t, out, "slack-channel")
}
...
func TestGetCmd_FactoryError(t *testing.T) {
	...
	err := cmd.RunE(cmd, []string{"slack-channel"})
	assert.EqualError(t, err, "factory failed")
}

func TestGetCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().GetObservabilityAlertsNotificationChannel(mock.Anything, mock.Anything, "slack-channel").Return(
		&gen.ObservabilityAlertsNotificationChannel{Metadata: gen.ObjectMeta{Name: "slack-channel"}}, nil,
	)

	cmd := newGetCmd(mockFactory(mc))
	require.NoError(t, cmd.Flags().Set("namespace", "acme-corp"))
	out := testutil.CaptureStdout(t, func() {
		require.NoError(t, cmd.RunE(cmd, []string{"slack-channel"}))
	})
	assert.Contains(t, out, "slack-channel")
}
...
func TestDeleteCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().DeleteObservabilityAlertsNotificationChannel(mock.Anything, mock.Anything, "slack-channel").Return(nil)
