...
// --- NewObservabilityPlaneCmd structure ---

func TestNewObservabilityPlaneCmd_Use(t *testing.T) {
	cmd := NewObservabilityPlaneCmd(errFactory("unused"))
...
func TestNewObservabilityPlaneCmd_Subcommands(t *testing.T) {
	cmd := NewObservabilityPlaneCmd(errFactory("unused"))
	names := make([]string, 0, len(cmd.Commands()))
	for _, sub := range cmd.Commands() {
		names = append(names, sub.Name())
	}
	assert.ElementsMatch(t, []string{"list", "get", "delete", "logs"}, names)
}
...
func TestListCmd_Success(t *testing.T) {
	...
	mc.EXPECT().ListObservabilityPlanes(mock.Anything, mock.Anything, mock.Anything).Return(&gen.ObservabilityPlaneList{
		Items:      []gen.ObservabilityPlane{{Metadata: gen.ObjectMeta{Name: "primary"}}},
...
func TestGetCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().GetObservabilityPlane(mock.Anything, mock.Anything, "primary").Return(
		&gen.ObservabilityPlane{Metadata: gen.ObjectMeta{Name: "primary"}}, nil,
	)
...
func TestDeleteCmd_MissingArg(t *testing.T) {
	...
	err := cmd.Args(cmd, []string{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "OBSERVABILITYPLANE_NAME")
}
...
func TestDeleteCmd_Success(t *testing.T) {
	mc := mocks.NewMockInterface(t)
	mc.EXPECT().DeleteObservabilityPlane(mock.Anything, mock.Anything, "primary").Return(nil)
