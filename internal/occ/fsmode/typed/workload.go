...
// GetContainers returns the container definitions as a map for template processing
func (w *Workload) GetContainers() map[string]interface{} {
	if len(w.Spec.Containers) == 0 {
		return nil
	}

	containers := make(map[string]interface{})
	for name, container := range w.Spec.Containers {
