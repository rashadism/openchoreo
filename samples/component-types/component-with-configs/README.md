...
### ComponentType (`web-service-with-configs`)

Defines a reusable component type template that supports configurations. It:

- Specifies the workload type as `deployment`
- Defines a schema with configurable parameters (replicas, port, resources, containerName)
- Declares environment-specific overrides for resource limits
- Templates the underlying Kubernetes resources (Deployment, Service, ConfigMaps, ExternalSecrets)
...
Defines the actual component using the `web-service-with-configs` type. It:

- References the `deployment/web-service-with-configs` component type
...
- Provides workload overrides for environment-specific configurations:
  ...
  - Environment-specific file content overrides

## How It Works

1. **ComponentType** acts as a template/blueprint that handles configurations
...
The OpenChoreo controller manager uses these resources and generates the Kubernetes resources (Deployment, Service, ConfigMaps, ExternalSecrets) based on the templates and parameters.
