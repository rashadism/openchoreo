...
kind: Deployment
metadata:
  name: demo-app-with-embedded-traits-development-<hash>
  namespace: dp-default-default-development-<hash>
  labels:
    openchoreo.dev/component: demo-app-with-embedded-traits
      ...
      volumes:
        - name: app-data
          persistentVolumeClaim:
            claimName: demo-app-with-embedded-traits-development-<hash>-data-storage
```
...
spec:
  ...
  hostnames:
    - development.openchoreoapis.localhost
  rules:
    - matches:
        - path:
            type: PathPrefix
            value: /demo-app-with-embedded-traits-development-<hash>
      backendRefs:
        - name: demo-app-with-embedded-traits-development-<hash>
          port: 80
```

### 4. HorizontalPodAutoscaler (from embedded trait)
...
spec:
  scaleTargetRef:
    ...
    kind: Deployment
    name: demo-app-with-embedded-traits-development-<hash>
  minReplicas: 2 # Locked by PE (cannot be overridden per environment)
  maxReplicas: 3 # Overridden by ReleaseBinding (default was 10)
  metrics:
    - type: Resource
      resource:
        name: cpu
...
### 5. PersistentVolumeClaim (from developer-added trait)

```yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: demo-app-with-embedded-traits-development-<hash>-data-storage
  namespace: dp-default-default-development-<hash>
spec:
  accessModes:
    - ReadWriteOnce
  resources:
    requests:
      storage: 1Gi # Overridden by ReleaseBinding
