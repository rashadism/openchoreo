...
All DataPlanes use cluster agent for secure communication.
...
Examples:
  ...
  # Multi-cluster with separate data plane
  ...
  $script_name --name org-a-dataplane --namespace org-a --plane-id shared-prod
  $script_name --name org-b-dataplane --namespace org-b --plane-id shared-prod

Note:
  - All DataPlanes use cluster agent for communication (mandatory)
  - Cluster agent establishes outbound WebSocket connection from data plane to control plane
  - No inbound ports need to be exposed on data plane clusters
EOF
}
...
# Validate dataplane context exists (if different from control plane)
if [ "$CONTROL_PLANE_CONTEXT" != "$DATAPLANE_CONTEXT" ]; then
  if ! kubectl config get-contexts "$DATAPLANE_CONTEXT" &>/dev/null; then
    error "Error: Data plane context '$DATAPLANE_CONTEXT' not found in kubeconfig"
    echo "Available contexts:"
    kubectl config get-contexts -o name
    exit 1
  fi
...
if [ "$DRY_RUN" = true ]; then
  ...
  if echo "$DATAPLANE_YAML" | kubectl --context="$CONTROL_PLANE_CONTEXT" apply -f - ; then
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Ensure cluster agent is deployed in the data plane cluster"
    echo "  2. Verify the agent connects to the control plane's cluster gateway"
    echo "  3. Check DataPlane status: kubectl --context=$CONTROL_PLANE_CONTEXT get dataplane $DATAPLANE_NAME -n $NAMESPACE"
    echo ""
  else
